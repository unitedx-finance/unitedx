const { address, encodeParameters, etherExp } = require("../Utils/Ethereum");
...
describe("CompoundLens", () => {
  ...
  describe("cTokenMetadataAll", () => {
    it("is correct for a cErc20 and cEther", async () => {
      ...
      expect(
        (
          await call(compoundLens, "cTokenMetadataAll", [
            [cErc20._address, cEth._address]
          ])
        ).map(cullTuple)
      ).toEqual([
        {
  ...
  describe("cTokenBalances", () => {
    it("is correct for cERC20", async () => {
      ...
      expect(
        cullTuple(
          await call(compoundLens, "cTokenBalances", [cErc20._address, acct])
        )
      ).toEqual({
        balanceOf: "0",
    ...
    it("is correct for cETH", async () => {
      ...
      expect(
        cullTuple(
          await call(compoundLens, "cTokenBalances", [cEth._address, acct], {
            gasPrice: "0"
          })
        )
      ).toEqual({
      ...
      expect(
        (
          await call(
            compoundLens,
            "cTokenBalancesAll",
            [[cErc20._address, cEth._address], acct],
            { gasPrice: "0" }
          )
        ).map(cullTuple)
      ).toEqual([
  ...
  describe("cTokenUnderlyingPrice", () => {
    it("gets correct price for cErc20", async () => {
      ...
      expect(
        cullTuple(
          await call(compoundLens, "cTokenUnderlyingPrice", [cErc20._address])
        )
      ).toEqual({
        cToken: cErc20._address,
        underlyingPrice: "0"
    ...
    it("gets correct price for cEth", async () => {
      ...
      expect(
        cullTuple(
          await call(compoundLens, "cTokenUnderlyingPrice", [cEth._address])
        )
      ).toEqual({
        cToken: cEth._address,
        underlyingPrice: "0"
  ...
  describe("cTokenUnderlyingPriceAll", () => {
    it("gets correct price for both", async () => {
      ...
      expect(
        (
          await call(compoundLens, "cTokenUnderlyingPriceAll", [
            [cErc20._address, cEth._address]
          ])
        ).map(cullTuple)
      ).toEqual([
  ...
  describe("getAccountLimits", () => {
    it("gets correct values", async () => {
      ...
      expect(
        cullTuple(
          await call(compoundLens, "getAccountLimits", [
  ...
  describe("governance", () => {
    ...
    beforeEach(async () => {
      ...
      gov = await deploy("GovernorAlpha", [
        address(0),
        comp._address,
        address(0)
      ]);
    ...
    describe("getGovReceipts", () => {
      it("gets correct values", async () => {
        expect(
          (
            await call(compoundLens, "getGovReceipts", [
              gov._address,
              acct,
              [proposalId]
            ])
    ...
    describe("getGovProposals", () => {
      it("gets correct values", async () => {
        expect(
          (
            await call(compoundLens, "getGovProposals", [
              gov._address,
              [proposalId]
            ])
          ).map(cullTuple)
  ...
  describe("comp", () => {
    ...
    describe("getCompBalanceMetadata", () => {
      it("gets correct values", async () => {
        expect(
          cullTuple(
            await call(compoundLens, "getCompBalanceMetadata", [
              comp._address,
              acct
            ])
          )
    ...
    describe("getCompBalanceMetadataExt", () => {
      it("gets correct values", async () => {
        ...
        expect(
          cullTuple(
            await call(compoundLens, "getCompBalanceMetadataExt", [
              comp._address,
              comptroller._address,
              acct
            ])
    ...
    describe("getCompVotes", () => {
      it("gets correct values", async () => {
        expect(
          (
            await call(compoundLens, "getCompVotes", [
              comp._address,
              acct,
      ...
      it("reverts on future value", async () => {
        await expect(
          call(compoundLens, "getCompVotes", [
            comp._address,
            acct,
            [currentBlock + 1]
          ])
