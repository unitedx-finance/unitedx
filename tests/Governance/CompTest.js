...
describe("Comp", () => {
  ...
  describe("balanceOf", () => {
    it("grants to initial account", async () => {
      expect(await call(comp, "balanceOf", [root])).toEqual(
  ...
  describe("delegateBySig", () => {
    ...
    it("reverts if the signatory is invalid", async () => {
      ...
      ).rejects.toRevert("revert Comp::delegateBySig: invalid signature");
    });
    ...
    it("reverts if the nonce is bad ", async () => {
      ...
      ).rejects.toRevert("revert Comp::delegateBySig: invalid nonce");
    });
    ...
    it("delegates on behalf of the signatory", async () => {
      ...
      const tx = await send(comp, "delegateBySig", [
        delegatee,
        nonce,
        expiry,
        v,
  ...
  describe("numCheckpoints", () => {
    it("returns the number of checkpoints for a delegate", async () => {
      ...
      await expect(call(comp, "checkpoints", [a1, 0])).resolves.toEqual(
        expect.objectContaining({
          fromBlock: t1.blockNumber.toString(),
          votes: "100"
        })
      );
      await expect(call(comp, "checkpoints", [a1, 1])).resolves.toEqual(
        expect.objectContaining({
          fromBlock: t2.blockNumber.toString(),
          votes: "90"
        })
      );
      await expect(call(comp, "checkpoints", [a1, 2])).resolves.toEqual(
        expect.objectContaining({
          fromBlock: t3.blockNumber.toString(),
          votes: "80"
        })
      );
      await expect(call(comp, "checkpoints", [a1, 3])).resolves.toEqual(
        expect.objectContaining({
  ...
  describe("getPriorVotes", () => {
    it("reverts if block number >= current block", async () => {
      await expect(call(comp, "getPriorVotes", [a1, 5e10])).rejects.toRevert(
    ...
    it("returns the latest block if >= last checkpoint block", async () => {
      ...
      expect(await call(comp, "getPriorVotes", [a1, t1.blockNumber])).toEqual(
        "10000000000000000000000000"
      );
      expect(
        await call(comp, "getPriorVotes", [a1, t1.blockNumber + 1])
      ).toEqual("10000000000000000000000000");
    });
    ...
    it("generally returns the voting balance at the appropriate checkpoint", async () => {
      ...
      expect(await call(comp, "getPriorVotes", [a1, t2.blockNumber])).toEqual(
        "9999999999999999999999990"
      );
      expect(
        await call(comp, "getPriorVotes", [a1, t2.blockNumber + 1])
      ).toEqual("9999999999999999999999990");
      expect(await call(comp, "getPriorVotes", [a1, t3.blockNumber])).toEqual(
        "9999999999999999999999980"
      );
      expect(
        await call(comp, "getPriorVotes", [a1, t3.blockNumber + 1])
      ).toEqual("9999999999999999999999980");
      expect(await call(comp, "getPriorVotes", [a1, t4.blockNumber])).toEqual(
        "10000000000000000000000000"
      );
      expect(
        await call(comp, "getPriorVotes", [a1, t4.blockNumber + 1])
      ).toEqual("10000000000000000000000000");
    });
  });
});
