...
describe("governorAlpha#castVote/2", () => {
  ...
  describe("We must revert if:", () => {
    it("There does not exist a proposal with matching proposal id where the current block number is between the proposal's start block (exclusive) and end block (inclusive)", async () => {
      await expect(call(gov, "castVote", [proposalId, true])).rejects.toRevert(
  ...
  describe("Otherwise", () => {
    it("we add the sender to the proposal's voters set", async () => {
      await expect(
        call(gov, "getReceipt", [proposalId, accounts[2]])
      ).resolves.toPartEqual({ hasVoted: false });
      await send(gov, "castVote", [proposalId, true], { from: accounts[2] });
      await expect(
        call(gov, "getReceipt", [proposalId, accounts[2]])
      ).resolves.toPartEqual({ hasVoted: true });
    });
    ...
    describe("and we take the balance returned by GetPriorVotes for the given sender and the proposal's start block, which may be zero,", () => {
      ...
      it("or AgainstVotes corresponding to the caller's support flag.", async () => {
        ...
        let beforeAgainsts = (await call(gov, "proposals", [proposalId]))
          .againstVotes;
        await mineBlock();
        await send(gov, "castVote", [proposalId, false], { from: actor });

        let afterAgainsts = (await call(gov, "proposals", [proposalId]))
          .againstVotes;
        expect(new BigNumber(afterAgainsts)).toEqual(
          new BigNumber(beforeAgainsts).plus(etherMantissa(400001))
        );
      });
    });
    ...
    describe("castVoteBySig", () => {
      ...
      it("reverts if the signatory is invalid", async () => {
        await expect(
          send(gov, "castVoteBySig", [proposalId, false, 0, "0xbad", "0xbad"])
        ).rejects.toRevert(
      ...
      it("casts vote on behalf of the signatory", async () => {
        ...
        let beforeFors = (await call(gov, "proposals", [proposalId])).forVotes;
        await mineBlock();
        const tx = await send(gov, "castVoteBySig", [
          proposalId,
          true,
          v,
          r,
    ...
    it("receipt uses one load", async () => {
      ...
      await saddle.trace(trxReceipt, {
        ...
        execLog: log => {
          ...
          let votes =
            "000000000000000000000000000000000000000054b419003bdf81640000";
          let voted = "01";
          let support = "01";
      ...
      await saddle.trace(trxReceipt2, {
        ...
        execLog: log => {
          let [output] = log.outputs;
          let votes =
            "0000000000000000000000000000000000000000a968320077bf02c80000";
          let voted = "01";
          let support = "00";
