...
const governorBravoPath = path.join(
  __dirname,
  "../../..",
  "contracts",
  "Governance/GovernorBravoInterfaces.sol"
);
...
describe("GovernorBravo#state/1", () => {
  ...
  it("Invalid for proposal not found", async () => {
    await expect(call(gov, "state", ["5"])).rejects.toRevert(
      "revert GovernorBravo::state: invalid proposal id"
    );
  });

  it("Pending", async () => {
    expect(await call(gov, "state", [trivialProposal.id], {})).toEqual(
      states["Pending"]
    );
  });
  ...
  it("Canceled", async () => {
    ...
    expect(await call(gov, "state", [+newProposalId])).toEqual(
      states["Canceled"]
    );
  });
