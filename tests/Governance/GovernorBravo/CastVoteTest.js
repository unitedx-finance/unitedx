...
describe("governorBravo#castVote/2", () => {
  ...
  describe("We must revert if:", () => {
    ...
    it("Such proposal already has an entry in its voters set matching the sender", async () => {
      ...
      let vote = await send(gov, "castVote", [proposalId, 1], {
        from: accounts[4]
      });

      let vote2 = await send(gov, "castVoteWithReason", [proposalId, 1, ""], {
        from: accounts[3]
      });
  ...
  describe("Otherwise", () => {
    ...
    describe("castVoteBySig", () => {
      ...
      it("reverts if the signatory is invalid", async () => {
        await expect(
          send(gov, "castVoteBySig", [proposalId, 0, 0, "0xbad", "0xbad"])
        ).rejects.toRevert(
