...
describe("admin / _setPendingAdmin / _acceptAdmin", () => {
  ...
  describe("_setPendingAdmin()", () => {
    it("should only be callable by admin", async () => {
      expect(
        await send(comptroller, "_setPendingAdmin", [accounts[0]], {
          from: accounts[0]
        })
    ...
    it("should properly set pending admin twice", async () => {
      ...
      expect(
        await send(comptroller, "_setPendingAdmin", [accounts[1]])
      ).toSucceed();
  ...
  describe("_acceptAdmin()", () => {
    it("should fail when pending admin is zero", async () => {
      expect(await send(comptroller, "_acceptAdmin")).toHaveTrollFailure(
        "UNAUTHORIZED",
    ...
    it("should emit log on success", async () => {
      ...
      const result = await send(comptroller, "_acceptAdmin", [], {
        from: accounts[0]
      });
