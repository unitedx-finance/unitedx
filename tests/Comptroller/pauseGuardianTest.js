  ...
  describe("_setPauseGuardian", () => {
    ...
    describe("failing", () => {
      it("emits a failure log if not sent by admin", async () => {
        let result = await send(comptroller, "_setPauseGuardian", [root], {
          from: accounts[1]
        });
      ...
      it("does not change the pause guardian", async () => {
        ...
        await send(comptroller, "_setPauseGuardian", [root], {
          from: accounts[1]
        });
  ...
  describe("setting paused", () => {
    ...
    describe("succeeding", () => {
      ...
      beforeEach(async () => {
        ...
        await send(comptroller, "_setPauseGuardian", [accounts[1]], {
          from: root
        });
      });
      ...
      globalMethods.forEach(async method => {
        it(`only pause guardian or admin can pause ${method}`, async () => {
          await expect(
            send(comptroller, `_set${method}Paused`, [true], {
              from: accounts[2]
            })
          ).rejects.toRevert("revert only pause guardian and admin can pause");
          await expect(
            send(comptroller, `_set${method}Paused`, [false], {
              from: accounts[2]
            })
          ).rejects.toRevert("revert only pause guardian and admin can pause");
        });
        ...
        it(`PauseGuardian can pause of ${method}GuardianPaused`, async () => {
          result = await send(comptroller, `_set${method}Paused`, [true], {
            from: pauseGuardian
          });
        ...
        it(`pauses ${method}`, async () => {
          await send(comptroller, `_set${method}Paused`, [true], {
            from: pauseGuardian
          });
          switch (method) {
            case "Transfer":
              await expect(
                send(comptroller, "transferAllowed", [
                  address(1),
                  address(2),
                  address(3),
                  1
    ...
    describe("succeeding", () => {
      ...
      marketMethods.forEach(async method => {
        it(`only pause guardian or admin can pause ${method}`, async () => {
          await expect(
            send(comptroller, `_set${method}Paused`, [cToken._address, true], {
              from: accounts[2]
            })
          ).rejects.toRevert("revert only pause guardian and admin can pause");
          await expect(
            send(comptroller, `_set${method}Paused`, [cToken._address, false], {
              from: accounts[2]
            })
          ).rejects.toRevert("revert only pause guardian and admin can pause");
        });
        ...
        it(`PauseGuardian can pause of ${method}GuardianPaused`, async () => {
          ...
          result = await send(comptroller, `_set${method}Paused`, [
            cToken._address,
            false
          ]);
        ...
        it(`pauses ${method}`, async () => {
          ...
          switch (method) {
            case "Mint":
              expect(
                await call(comptroller, "mintAllowed", [
                  address(1),
                  address(2),
                  1
                ])
            ...
            case "Borrow":
              expect(
                await call(comptroller, "borrowAllowed", [
                  address(1),
                  address(2),
                  1
                ])
              ...
              await expect(
                send(comptroller, "borrowAllowed", [
                  cToken._address,
                  address(2),
                  1
                ])
