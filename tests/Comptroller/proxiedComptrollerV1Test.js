  ...
  describe("delegating to comptroller v1", () => {
    ...
    describe("becoming brains sets initial state", () => {
      ...
      it("on success it sets closeFactor and maxAssets as specified", async () => {
        const comptroller = await initializeBrains(
          oracle,
          closeFactor,
          maxAssets
        );
      ...
      it("on reinitialization success, it doesn't set closeFactor or maxAssets", async () => {
        let comptroller = await initializeBrains(
          oracle,
          closeFactor,
          maxAssets
        );
    ...
    describe("_setCollateralFactor", () => {
      ...
      it("fails if asset is not listed", async () => {
        expect(
          await send(unitrollerAsComptroller, "_setCollateralFactor", [
            cToken._address,
            half
          ])
        ).toHaveTrollFailure(
    ...
    describe("_supportMarket", () => {
      ...
      it("fails if asset is not a CToken", async () => {
        ...
        await expect(
          send(unitrollerAsComptroller, "_supportMarket", [notACToken._address])
        ).rejects.toRevert();
      });

      it("succeeds and sets market", async () => {
        const result = await send(unitrollerAsComptroller, "_supportMarket", [
          cToken._address
        ]);
      ...
      it("cannot list a market a second time", async () => {
        const result1 = await send(unitrollerAsComptroller, "_supportMarket", [
          cToken._address
        ]);
        const result2 = await send(unitrollerAsComptroller, "_supportMarket", [
          cToken._address
        ]);
