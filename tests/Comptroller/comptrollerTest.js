const { etherMantissa, both } = require("../Utils/Ethereum");
...
describe("Comptroller", () => {
  ...
  describe("constructor", () => {
    ...
    it("on success it sets closeFactor as specified", async () => {
      const comptroller = await makeComptroller();
      expect(await call(comptroller, "closeFactorMantissa")).toEqualNumber(
        0.051e18
      );
    });
  });
  ...
  describe("_setLiquidationIncentive", () => {
    ...
    it("fails if called by non-admin", async () => {
      ...
      expect(
        await call(comptroller, "liquidationIncentiveMantissa")
  ...
  describe("_setPriceOracle", () => {
    ...
    it("fails if called by non-admin", async () => {
      expect(
        await send(comptroller, "_setPriceOracle", [newOracle._address], {
          from: accounts[0]
        })
      ).toHaveTrollFailure("UNAUTHORIZED", "SET_PRICE_ORACLE_OWNER_CHECK");
      expect(await comptroller.methods.oracle().call()).toEqual(
        oldOracle._address
      );
    });
    ...
    it.skip("reverts if passed a contract that doesn't implement isPriceOracle", async () => {
      await expect(
        send(comptroller, "_setPriceOracle", [comptroller._address])
      ).rejects.toRevert();
    ...
    it.skip("reverts if passed a contract that implements isPriceOracle as false", async () => {
      ...
      await expect(
        send(notOracle, "_setPriceOracle", [comptroller._address])
    ...
    it("accepts a valid price oracle and emits a NewPriceOracle event", async () => {
      const result = await send(comptroller, "_setPriceOracle", [
        newOracle._address
      ]);
  ...
  describe("_setCollateralFactor", () => {
    ...
    it("fails if asset is not listed", async () => {
      const cToken = await makeCToken();
      expect(
        await send(cToken.comptroller, "_setCollateralFactor", [
          cToken._address,
          half
        ])
      ).toHaveTrollFailure(
    ...
    it("succeeds and sets market", async () => {
      ...
      const result = await send(cToken.comptroller, "_setCollateralFactor", [
        cToken._address,
        half
      ]);
  ...
  describe("_supportMarket", () => {
    ...
    it("fails if asset is not a CToken", async () => {
      ...
      await expect(
        send(comptroller, "_supportMarket", [asset._address])
      ).rejects.toRevert();
    });

    it("succeeds and sets market", async () => {
      const cToken = await makeCToken();
      const result = await send(cToken.comptroller, "_supportMarket", [
        cToken._address
      ]);
    ...
    it("cannot list a market a second time", async () => {
      const cToken = await makeCToken();
      const result1 = await send(cToken.comptroller, "_supportMarket", [
        cToken._address
      ]);
      const result2 = await send(cToken.comptroller, "_supportMarket", [
        cToken._address
      ]);
      expect(result1).toHaveLog("MarketListed", { cToken: cToken._address });
      expect(result2).toHaveTrollFailure(
    ...
    it("can list two different markets", async () => {
      ...
      const result1 = await send(cToken1.comptroller, "_supportMarket", [
        cToken1._address
      ]);
      const result2 = await send(cToken1.comptroller, "_supportMarket", [
        cToken2._address
      ]);
  ...
  describe("redeemVerify", () => {
    it("should allow you to redeem 0 underlying for 0 tokens", async () => {
      ...
      await call(comptroller, "redeemVerify", [
        cToken._address,
        accounts[0],
        0,
        0
    ...
    it("should not allow you to redeem 5 underlying for 0 tokens", async () => {
      ...
      const cToken = await makeCToken({ comptroller: comptroller });
      await expect(
        call(comptroller, "redeemVerify", [cToken._address, accounts[0], 5, 0])
