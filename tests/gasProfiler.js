const { etherUnsigned, etherMantissa, etherExp } = require("./Utils/Ethereum");
...
describe("Gas report", () => {
  ...
  describe("CToken", () => {
    ...
    it("second mint", async () => {
      ...
      expect(Object.keys(mint2Receipt.events)).toEqual([
        "AccrueInterest",
        "Transfer",
        "Mint"
      ]);
  ...
  describe.each([["unitroller-g6"], ["unitroller"]])(
    "Comp claims %s",
    patch => {
      beforeEach(async () => {
        [root, minter, redeemer, ...accounts] = saddle.accounts;
        ...
        if (patch == "unitroller") {
          ...
          await send(comptroller, "_addCompMarkets", [
            [cToken].map(c => c._address)
          ]);
          await send(comptroller, "setCompSpeed", [
            cToken._address,
            etherExp(0.05)
          ]);
        }
      ...
      it(`${patch} second mint with comp accrued`, async () => {
        ...
        console.log(
          "Comp balance before mint",
          (await compBalance(comptroller, minter)).toString()
        );
        console.log(
          "Comp accrued before mint",
          (await compAccrued(comptroller, minter)).toString()
        );
        const mint2Receipt = await mint(
          cToken,
          minter,
          mintAmount,
          exchangeRate
        ...
        console.log(
          "Comp balance after mint",
          (await compBalance(comptroller, minter)).toString()
        );
        console.log(
          "Comp accrued after mint",
          (await compAccrued(comptroller, minter)).toString()
        );
        recordGasCost(
          mint2Receipt.gasUsed,
          `${patch} second mint with comp accrued`,
          filename
        );
      });
      ...
      it(`${patch} claim comp`, async () => {
        ...
        console.log(
          "Comp balance before claim",
          (await compBalance(comptroller, minter)).toString()
        );
        console.log(
          "Comp accrued before claim",
          (await compAccrued(comptroller, minter)).toString()
        );
        const claimReceipt = await claimComp(comptroller, minter);
        console.log(
          "Comp balance after claim",
          (await compBalance(comptroller, minter)).toString()
        );
        console.log(
          "Comp accrued after claim",
          (await compAccrued(comptroller, minter)).toString()
        );
