...
function baseRoofRateFn(
  ...
  if (baseRatePerBlock < stabilityFeePerBlock) {
    multiplierPerBlock =
      (stabilityFeePerBlock - baseRatePerBlock + gapPerBlock) / kink;
  } else {
  ...
  if (ur <= kink) {
    ...
    return (
      excessUtil * jumpPerBlock + kink * multiplierPerBlock + baseRatePerBlock
    );
  }
}

function daiSupplyRate(
  dsr,
  duty,
  mkrBase,
  jump,
  ...
  const dsrPerBlock = (dsr - 1) * 15;
  const ur = utilizationRate(cash, borrows, reserves);
  const borrowRate = baseRoofRateFn(
    dsr,
    duty,
    mkrBase,
    jump,
  ...
  const cashSupplyRate = new BigNum(cash)
    .times(new BigNum(dsrPerBlock))
    .div(underlying);
...
describe("DAIInterestRateModelV3", () => {
  describe("constructor", () => {
    it("sets jug and ilk address and pokes", async () => {
      ...
      let sr = await call(model, "getSupplyRate", [
        ...args,
        etherUnsigned(0.1e18)
      ]);
    ...
    [
      ...
      .forEach(
        ([
          ...
          it(`calculates correct borrow value for dsr=${dsr /
            1e25}%, duty=${duty / 1e25}%, base=${base / 1e25}%, jump=${jump /
            1e18}, cash=${cash}, borrows=${borrows}, reserves=${reserves}`, async () => {
            ...
            const pot = await deploy("MockPot", [
    ...
    [
      ...
      .forEach(
        ([
          ...
          it(`calculates correct supply value for dsr=${dsr /
            1e25}%, duty=${duty / 1e25}%, base=${base /
            1e25}%, cash=${cash}, borrows=${borrows}, reserves=${reserves}`, async () => {
            const onePlusPerSecondDsr = 1e27 + dsr / secondsPerYear;
            const onePlusPerSecondDuty = 1e27 + duty / secondsPerYear;
            const perSecondBase = base / secondsPerYear;
            ...
            const actual = await getSupplyRate(
              daiIRM,
              cash,
              borrows,
              reserves,
