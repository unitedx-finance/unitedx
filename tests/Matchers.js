...
function revertWithCustomError(actual, errorName, errorArgs, reporter) {
  const correctMessage =
    !!actual["message"] &&
    actual.message === `VM Exception while processing transaction: revert`;
  ...
  if (correctMessage && !!err) {
    ...
    matchingError =
      Object.values(actual.results).findIndex(
        v => v.error === "revert" && v.return === expectedReturn
      ) >= 0;
  }
  return {
...
expect.extend({
  ...
  toEqualNumber(actual, expected) {
    return match.call(
      ...
      actual,
      expected,
      etherUnsigned(actual).isEqualTo(etherUnsigned(expected)),
      opts("toEqualNumber")
    );
  },
  ...
  toRevertWithCustomError(
    actual,
    errorName,
    errorArgs = [],
    reporter = TokenErr
  ) {
    return revertWithCustomError.call(
      this,
      actual,
      errorName,
      errorArgs,
