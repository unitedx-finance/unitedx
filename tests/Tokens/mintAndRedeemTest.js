...
async function preRedeem(
  ...
  await send(cToken.underlying, "harnessSetFailTransferToAddress", [
    redeemer,
    false
  ]);
  await send(cToken, "harnessSetExchangeRate", [etherMantissa(exchangeRate)]);
...
describe("CToken", function() {
  ...
  describe("mintFresh", () => {
    ...
    it("fails if comptroller tells it to", async () => {
      ...
      ).rejects.toRevertWithCustomError("MintComptrollerRejection", [11]);
    });
    ...
    it("fails if not fresh", async () => {
      ...
      await expect(
        mintFresh(cToken, minter, mintAmount)
      ).rejects.toRevertWithCustomError("MintFreshnessCheck");
    });
    ...
    it("fails if insufficient approval", async () => {
      expect(
        await send(cToken.underlying, "approve", [cToken._address, 1], {
          from: minter
        })
      ).toSucceed();
  ...
  describe("mint", () => {
    ...
    it("emits a mint failure if interest accrual fails", async () => {
      ...
      await expect(quickMint(cToken, minter, mintAmount)).rejects.toRevert(
    ...
    it("emits an AccrueInterest event", async () => {
      expect(await quickMint(cToken, minter, mintAmount)).toHaveLog(
        "AccrueInterest",
        {
  ...
  [redeemFreshTokens, redeemFreshAmount].forEach(redeemFresh => {
    describe(redeemFresh.name, () => {
      ...
      it("fails if comptroller tells it to", async () => {
        ...
        await expect(
          redeemFresh(cToken, redeemer, redeemTokens, redeemAmount)
        ).rejects.toRevertWithCustomError("RedeemComptrollerRejection", [11]);
      });
      ...
      it("fails if insufficient protocol cash to transfer out", async () => {
        ...
        await expect(
          redeemFresh(cToken, redeemer, redeemTokens, redeemAmount)
        ).rejects.toRevertWithCustomError("RedeemTransferOutNotPossible");
      });

      it("fails if exchange calculation fails", async () => {
        if (redeemFresh == redeemFreshTokens) {
          expect(
            await send(cToken, "harnessSetExchangeRate", [UInt256Max()])
          ).toSucceed();
          await expect(
            redeemFresh(cToken, redeemer, redeemTokens, redeemAmount)
          ).rejects.toRevert();
  ...
  describe("redeem", () => {
    ...
    it("emits a redeem failure if interest accrual fails", async () => {
      await send(cToken.interestRateModel, "setFailBorrowRate", [true]);
      await expect(
        quickRedeem(cToken, redeemer, redeemTokens)
    ...
    it("returns error from redeemFresh without emitting any extra logs", async () => {
      ...
      await expect(
        quickRedeem(cToken, redeemer, redeemTokens, { exchangeRate })
      ).rejects.toRevert();
    });
    ...
    it("returns success from redeemFresh and redeems the right amount", async () => {
      ...
      expect(await balanceOf(cToken.underlying, redeemer)).toEqualNumber(
        redeemAmount
      );
    });
    ...
    it("returns success from redeemFresh and redeems the right amount of underlying", async () => {
      ...
      expect(
        await quickRedeemUnderlying(cToken, redeemer, redeemAmount, {
          exchangeRate
        })
      ).toSucceed();
      expect(redeemAmount).not.toEqualNumber(0);
