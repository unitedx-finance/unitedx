...
async function preLiquidate(
  ...
  await send(cToken.underlying, "harnessSetFailTransferFromAddress", [
    liquidator,
    false
  ]);
  await send(cToken.interestRateModel, "setFailBorrowRate", [false]);
  await send(cTokenCollateral.interestRateModel, "setFailBorrowRate", [false]);
  await send(cTokenCollateral.comptroller, "setCalculatedSeizeTokens", [
    seizeTokens
  ]);
...
async function liquidateFresh(
  ...
  return send(cToken, "harnessLiquidateBorrowFresh", [
    liquidator,
    borrower,
    repayAmount,
...
async function liquidate(
  ...
  return send(
    cToken,
    "liquidateBorrow",
    [borrower, repayAmount, cTokenCollateral._address],
    { from: liquidator }
  );
}
...
describe("CToken", function() {
  ...
  const exchangeRate = etherExp(0.2);

  const protocolShareTokens = seizeTokens
    .multipliedBy(protocolSeizeShareMantissa)
    .dividedBy(etherExp(1));
  const liquidatorShareTokens = seizeTokens.minus(protocolShareTokens);

  const addReservesAmount = protocolShareTokens
  ...
  describe("liquidateBorrowFresh", () => {
    ...
    it("fails if repayAmount = 0", async () => {
      await expect(
        liquidateFresh(cToken, liquidator, borrower, 0, cTokenCollateral)
    ...
    it("fails if repay fails", async () => {
      ...
      ).rejects.toRevertWithCustomError("RepayBorrowComptrollerRejection", [
        11
      ]);
    });
    ...
    it("reverts if seize fails", async () => {
      ...
      ).rejects.toRevertWithCustomError("LiquidateSeizeComptrollerRejection", [
        11
      ]);
    });
  ...
  describe("liquidateBorrow", () => {
    it("emits a liquidation failure if borrowed asset interest accrual fails", async () => {
      ...
      await expect(
        liquidate(cToken, liquidator, borrower, repayAmount, cTokenCollateral)
    ...
    it("emits a liquidation failure if collateral asset interest accrual fails", async () => {
      await send(cTokenCollateral.interestRateModel, "setFailBorrowRate", [
        true
      ]);
      await expect(
        liquidate(cToken, liquidator, borrower, repayAmount, cTokenCollateral)
    ...
    it("returns error from liquidateBorrowFresh without emitting any extra logs", async () => {
      await expect(
        liquidate(cToken, liquidator, borrower, 0, cTokenCollateral)
      ).rejects.toRevertWithCustomError("LiquidateCloseAmountIsZero");
    });
  ...
  describe("seize", () => {
    ...
    it("fails if seize is not allowed", async () => {
      await send(cToken.comptroller, "setSeizeAllowed", [false]);
      await expect(
        seize(cTokenCollateral, liquidator, borrower, seizeTokens)
      ).rejects.toRevertWithCustomError("LiquidateSeizeComptrollerRejection", [
        11
      ]);
    });
...
describe("Comptroller", () => {
  it("liquidateBorrowAllowed allows deprecated markets to be liquidated", async () => {
    ...
    let collatAmount = 10;
    let borrowAmount = 2;
    const cTokenCollat = await makeCToken({
      supportMarket: true,
      underlyingPrice: 1,
      collateralFactor: 0.5
    });
    const cTokenBorrow = await makeCToken({
      supportMarket: true,
      underlyingPrice: 1,
      comptroller: cTokenCollat.comptroller
    });
    const comptroller = cTokenCollat.comptroller;

    // borrow some tokens
    await send(cTokenCollat.underlying, "harnessSetBalance", [
      borrower,
      collatAmount
    ]);
    await send(
      cTokenCollat.underlying,
      "approve",
      [cTokenCollat._address, collatAmount],
      { from: borrower }
    );
    await send(cTokenBorrow.underlying, "harnessSetBalance", [
      cTokenBorrow._address,
      collatAmount
    ]);
    ...
    expect(
      await send(cTokenCollat, "mint", [collatAmount], { from: borrower })
    ).toSucceed();
    expect(
      await send(cTokenBorrow, "borrow", [borrowAmount], { from: borrower })
    ).toSucceed();

    // show the account is healthy
    expect(
      await call(comptroller, "isDeprecated", [cTokenBorrow._address])
    ).toEqual(false);
    expect(
      await call(comptroller, "liquidateBorrowAllowed", [
        cTokenBorrow._address,
        cTokenCollat._address,
        liquidator,
        borrower,
        borrowAmount
      ])
    ).toHaveTrollError("INSUFFICIENT_SHORTFALL");
    ...
    expect(
      await send(comptroller, "_setBorrowPaused", [cTokenBorrow._address, true])
    ).toSucceed();
    expect(
      await send(cTokenBorrow, "_setReserveFactor", [etherMantissa(1)])
    ).toSucceed();
    ...
    expect(
      await send(comptroller, "liquidateBorrowAllowed", [
        cTokenBorrow._address,
        cTokenCollat._address,
        liquidator,
        borrower,
