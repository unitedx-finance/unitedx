...
describe("CToken", function() {
  ...
  describe("constructor", () => {
    it("fails when non erc-20 underlying", async () => {
      await expect(
        makeCToken({ underlying: { _address: root } })
      ).rejects.toRevert("revert");
    });

    it("fails when 0 initial exchange rate", async () => {
      await expect(makeCToken({ exchangeRate: 0 })).rejects.toRevert(
    ...
    it("succeeds with erc-20 underlying and non-zero exchange rate", async () => {
      ...
      expect(await call(cToken, "underlying")).toEqual(
  ...
  describe("balanceOfUnderlying", () => {
    it("has an underlying balance", async () => {
      ...
      expect(await call(cToken, "balanceOfUnderlying", [root])).toEqualNumber(
        200
      );
    });
  });
  ...
  describe("supplyRatePerBlock", () => {
    ...
    it("has a supply rate", async () => {
      ...
      await send(cToken, "harnessSetReserveFactorFresh", [etherMantissa(0.01)]);
  ...
  describe("borrowBalanceCurrent", () => {
    ...
    beforeEach(async () => {
      await setBorrowRate(cToken, 0.001);
    ...
    it("reverts if interest accrual fails", async () => {
      ...
      await expect(
        send(cToken, "borrowBalanceCurrent", [borrower])
    ...
    it("returns successful result from borrowBalanceStored with no interest", async () => {
      ...
      expect(
        await call(cToken, "borrowBalanceCurrent", [borrower])
      ).toEqualNumber(5e18);
    });
  ...
  describe("borrowBalanceStored", () => {
    ...
    it("returns 0 for account with no borrows", async () => {
      expect(
        await call(cToken, "borrowBalanceStored", [borrower])
      ).toEqualNumber(0);
    });
    ...
    it("reverts on overflow of principal", async () => {
      ...
      await expect(
        call(cToken, "borrowBalanceStored", [borrower])
      ).rejects.toRevert();
    });
  ...
  describe("exchangeRateStored", () => {
    ...
    it("calculates with cTokenSupply and total borrows", async () => {
      const cTokenSupply = 100e18,
        totalBorrows = 10e18,
        totalReserves = 0;
      await send(
        cToken,
        "harnessExchangeRateDetails",
        [cTokenSupply, totalBorrows, totalReserves].map(etherUnsigned)
      );
      const result = await call(cToken, "exchangeRateStored");
      expect(result).toEqualNumber(etherMantissa(0.1));
    });

    it("calculates with cash and cTokenSupply", async () => {
      const cTokenSupply = 5e18,
        totalBorrows = 0,
        totalReserves = 0;
      expect(
        await send(cToken.underlying, "transfer", [
          cToken._address,
          etherMantissa(500)
        ])
      ).toSucceed();
