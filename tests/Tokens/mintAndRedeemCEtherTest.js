  ...
  [mintExplicit, mintFallback].forEach(mint => {
    describe(mint.name, () => {
      ...
      it("reverts if interest accrual fails", async () => {
        ...
        await expect(mint(cToken, minter, mintAmount)).rejects.toRevert(
        ...
        expect(afterBalances).toEqual(
          await adjustBalances(beforeBalances, [
            ...
            [
              ...
              minter,
              "eth",
              -mintAmount.plus(await etherGasCost(receipt))
            ],
  ...
  [redeemCTokens, redeemUnderlying].forEach(redeem => {
    describe(redeem.name, () => {
      ...
      it("emits a redeem failure if interest accrual fails", async () => {
        await send(cToken.interestRateModel, "setFailBorrowRate", [true]);
        await expect(
          redeem(cToken, redeemer, redeemTokens, redeemAmount)
        ...
        expect(afterBalances).toEqual(
          await adjustBalances(beforeBalances, [
            ...
            [
              ...
              redeemer,
              "eth",
              redeemAmount.minus(await etherGasCost(receipt))
            ],
