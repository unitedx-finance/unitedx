...
describe("CToken", function() {
  ...
  describe("_setInterestRateModelFresh", () => {
    ...
    it("fails if called by non-admin", async () => {
      await expect(
        send(cToken, "harnessSetInterestRateModelFresh", [newModel._address], {
          from: accounts[0]
        })
      ).rejects.toRevertWithCustomError("SetInterestRateModelOwnerCheck");
      expect(await call(cToken, "interestRateModel")).toEqual(
        oldModel._address
      );
    });
    ...
    it("reverts if passed a contract that doesn't implement isInterestRateModel", async () => {
      await expect(
        send(cToken, "harnessSetInterestRateModelFresh", [
    ...
    it("reverts if passed a contract that implements isInterestRateModel as false", async () => {
      ...
      await expect(
        send(cToken, "harnessSetInterestRateModelFresh", [badModel._address])
    ...
    it("accepts new valid interest rate model", async () => {
      expect(
        await send(cToken, "harnessSetInterestRateModelFresh", [
          newModel._address
        ])
      ).toSucceed();
      expect(await call(cToken, "interestRateModel")).toEqual(
    ...
    it("emits expected log when accepting a new valid interest rate model", async () => {
      const result = await send(cToken, "harnessSetInterestRateModelFresh", [
        newModel._address
      ]);
  ...
  describe("_setInterestRateModel", () => {
    ...
    it("emits a set market interest rate model failure if interest accrual fails", async () => {
      ...
      await expect(
        send(cToken, "_setInterestRateModel", [newModel._address])
    ...
    it("reports success when _setInterestRateModelFresh succeeds", async () => {
      const { reply, receipt } = await both(cToken, "_setInterestRateModel", [
        newModel._address
      ]);
