...
describe("CToken", function() {
  ...
  describe("_setReserveFactorFresh", () => {
    ...
    it("rejects change by non-admin", async () => {
      await expect(
        send(cToken, "harnessSetReserveFactorFresh", [factor], {
          from: accounts[0]
        })
    ...
    it("rejects change if market not fresh", async () => {
      ...
      await expect(
        send(cToken, "harnessSetReserveFactorFresh", [factor])
      ).rejects.toRevertWithCustomError("SetReserveFactorFreshCheck");
      expect(await call(cToken, "reserveFactorMantissa")).toEqualNumber(0);
    });

    it("rejects newReserveFactor that descales to 1", async () => {
      await expect(
        send(cToken, "harnessSetReserveFactorFresh", [etherMantissa(1.01)])
      ).rejects.toRevertWithCustomError("SetReserveFactorBoundsCheck");
      expect(await call(cToken, "reserveFactorMantissa")).toEqualNumber(0);
    });

    it("accepts newReserveFactor in valid range and emits log", async () => {
      const result = await send(cToken, "harnessSetReserveFactorFresh", [
        factor
      ]);
    ...
    it("accepts a change back to zero", async () => {
      const result1 = await send(cToken, "harnessSetReserveFactorFresh", [
        factor
      ]);
  ...
  describe("_setReserveFactor", () => {
    ...
    it("emits a reserve factor failure if interest accrual fails", async () => {
      ...
      await expect(
        send(cToken, "_setReserveFactor", [factor])
    ...
    it("returns error from setReserveFactorFresh without emitting any extra logs", async () => {
      await expect(
        send(cToken, "_setReserveFactor", [etherMantissa(2)])
      ).rejects.toRevert();
      expect(await call(cToken, "reserveFactorMantissa")).toEqualNumber(0);
    });
  ...
  describe("_reduceReservesFresh", () => {
    ...
    beforeEach(async () => {
      cToken = await makeCToken();
      expect(
        await send(cToken, "harnessSetTotalReserves", [reserves])
      ).toSucceed();
      expect(
    ...
    it("fails if called by non-admin", async () => {
      await expect(
        send(cToken, "harnessReduceReservesFresh", [reduction], {
          from: accounts[0]
        })
    ...
    it("fails if market not fresh", async () => {
      ...
      await expect(
        send(cToken, "harnessReduceReservesFresh", [reduction])
      ).rejects.toRevertWithCustomError("ReduceReservesFreshCheck");
      expect(await call(cToken, "totalReserves")).toEqualNumber(reserves);
    });

    it("fails if amount exceeds reserves", async () => {
      await expect(
        send(cToken, "harnessReduceReservesFresh", [reserves.plus(1)])
      ).rejects.toRevertWithCustomError("ReduceReservesCashValidation");
      expect(await call(cToken, "totalReserves")).toEqualNumber(reserves);
    });
    ...
    it("fails if amount exceeds available cash", async () => {
      ...
      await expect(
        send(cToken, "harnessReduceReservesFresh", [reserves])
      ).rejects.toRevertWithCustomError("ReduceReservesCashNotAvailable");
      expect(await call(cToken, "totalReserves")).toEqualNumber(reserves);
    });

    it("increases admin balance and reduces reserves on success", async () => {
      const balance = etherUnsigned(
        await call(cToken.underlying, "balanceOf", [root])
      );
      expect(
        await send(cToken, "harnessReduceReservesFresh", [reserves])
      ).toSucceed();
      expect(await call(cToken.underlying, "balanceOf", [root])).toEqualNumber(
    ...
    it("emits an event on success", async () => {
      const result = await send(cToken, "harnessReduceReservesFresh", [
        reserves
      ]);
  ...
  describe("_reduceReserves", () => {
    ...
    it("emits a reserve-reduction failure if interest accrual fails", async () => {
      ...
      await fastForward(cToken, 1);
      await expect(
        send(cToken, "_reduceReserves", [reduction])
      ).rejects.toRevert("revert INTEREST_RATE_MODEL_ERROR");
    });
  ...
  describe("CEther addReserves", () => {
    ...
    it("add reserves for CEther", async () => {
      ...
      expect(balanceAfter).toEqual(
        await adjustBalances(balanceBefore, [
