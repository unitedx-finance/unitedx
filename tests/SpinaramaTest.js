...
describe("Spinarama", () => {
  ...
  describe("#repayRepay", () => {
    it("should succeed", async () => {
      ...
      await send(cToken1.underlying, "approve", [cToken1._address, 10], {
        from
      });
      await send(cToken2.underlying, "harnessSetBalance", [
        cToken2._address,
        10
      ]);
      await send(cToken2, "harnessSetTotalSupply", [100]);
      await send(cToken2.underlying, "approve", [cToken2._address, 10], {
        from
      });
