...
const [ComptrollerErrorReporter, TokenErrorReporter] = contents.children.filter(
  k => k.type === "ContractDefinition"
);
...
function parse(reporter) {
  const Error = (
    reporter.subNodes.find(k => k.name == "Error") || { members: [] }
  ).members.reduce((obj, i, idx) => {
    obj[i.name] = `${idx}`;
    return obj;
  }, {});
  const FailureInfo = (
    reporter.subNodes.find(k => k.name == "FailureInfo") || { members: [] }
  ).members.reduce((obj, i, idx) => {
    obj[i.name] = `${idx}`;
    return obj;
  }, {});
  const CustomErrors = reporter.subNodes
    .filter(k => k.type === "CustomErrorDefinition")
    .reduce((obj, i) => {
      obj[i.name] = {
        type: "function",
        name: i.name,
