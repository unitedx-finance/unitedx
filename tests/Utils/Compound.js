...
async function makeComptroller(opts = {}) {
  ...
  if (kind == "unitroller-g3") {
    ...
    await send(unitroller, "_setPendingImplementation", [comptroller._address]);
    await send(comptroller, "_become", [
      unitroller._address,
      compRate,
      compMarkets,
      otherMarkets
...
async function makeCToken(opts = {}) {
  const { root = saddle.account, kind = "cerc20" } = opts || {};
  ...
  switch (kind) {
    ...
    case "cdai":
      ...
      cDelegator = await deploy("CErc20Delegator", [
        ...
        encodeParameters(
          ["address", "address"],
          [cDaiMaker._address, cDaiMaker._address]
        )
      ]);
      cToken = await saddle.getContractAt(
        "CDaiDelegateHarness",
        cDelegator._address
      );
      break;
  ...
  if (opts.underlyingPrice) {
    ...
    await send(comptroller.priceOracle, "setUnderlyingPrice", [
      cToken._address,
      price
    ]);
  }
...
async function adjustBalances(balances, deltas) {
  for (let delta of deltas) {
    ...
    balances[cToken._address][account][key] = new BigNumber(
...
async function preApprove(cToken, from, amount, opts = {}) {
  if (dfn(opts.faucet, true)) {
    expect(
      await send(cToken.underlying, "harnessSetBalance", [from, amount], {
        from
      })
    ).toSucceed();
  }

  return send(cToken.underlying, "approve", [cToken._address, amount], {
    from
  });
}
