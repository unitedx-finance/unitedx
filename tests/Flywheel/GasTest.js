...
const { etherExp, etherDouble, etherUnsigned } = require("../Utils/Ethereum");
...
describe.skip("Flywheel trace ops", () => {
  ...
  it("update supply index SSTOREs", async () => {
    ...
    const tx = await send(comptroller, "harnessUpdateCompSupplyIndex", [
      market._address
    ]);
  ...
  it("update borrow index SSTOREs", async () => {
    ...
    const tx = await send(comptroller, "harnessUpdateCompBorrowIndex", [
      market._address,
      etherExp(1.1)
    ]);
