...
async function totalCompAccrued(comptroller, user) {
  return (await compAccrued(comptroller, user)).plus(
    await compBalance(comptroller, user)
  );
}
...
describe("Flywheel upgrade", () => {
  describe("becomes the comptroller", () => {
    it("adds the comp markets", async () => {
      ...
      let compMarkets = await Promise.all(
        [1, 2, 3].map(async _ => {
    ...
    it("adds the other markets", async () => {
      ...
      expect(await call(unitroller, "getCompMarkets")).toEqual(
        allMarkets.slice(0, 1)
      );
    });
    ...
    it("_supportMarket() adds to all markets, and only once", async () => {
      ...
      for (let _ of Array(10)) {
        allMarkets.push(
          await makeCToken({ comptroller: unitroller, supportMarket: true })
        );
      }
      expect(await call(unitroller, "getAllMarkets")).toEqual(
        allMarkets.map(c => c._address)
      );
      expect(
        makeComptroller({
          kind: "unitroller-g3",
...
describe("Flywheel", () => {
  ...
  beforeEach(async () => {
    ...
    cLOW = await makeCToken({
      comptroller,
      supportMarket: true,
      underlyingPrice: 1,
      interestRateModelOpts
    });
    cREP = await makeCToken({
      comptroller,
      supportMarket: true,
      underlyingPrice: 2,
      interestRateModelOpts
    });
    cZRX = await makeCToken({
      comptroller,
      supportMarket: true,
      underlyingPrice: 3,
      interestRateModelOpts
    });
    cEVIL = await makeCToken({
      comptroller,
      supportMarket: false,
      underlyingPrice: 3,
      interestRateModelOpts
    });
    cUSD = await makeCToken({
      comptroller,
      supportMarket: true,
      underlyingPrice: 1,
  ...
  describe("_grantComp()", () => {
    beforeEach(async () => {
      await send(
        comptroller.comp,
        "transfer",
        [comptroller._address, etherUnsigned(50e18)],
        { from: root }
      );
    });
  ...
  describe("getCompMarkets()", () => {
    it("should return the comp markets", async () => {
      for (let mkt of [cLOW, cREP, cZRX]) {
        await send(comptroller, "_setCompSpeeds", [
          [mkt._address],
          [etherExp(0.5)],
          [etherExp(0.5)]
        ]);
  ...
  describe("_setCompSpeeds()", () => {
    ...
    it("should correctly drop a comp market if called by admin", async () => {
      ...
      const tx = await send(comptroller, "_setCompSpeeds", [
        [cLOW._address],
        [0],
        [0]
      ]);
  ...
  describe("updateCompBorrowIndex()", () => {
    it("should calculate comp borrower index correctly", async () => {
      ...
      */
      ...
      const { index, block } = await call(comptroller, "compBorrowState", [
        mkt._address
      ]);
    ...
    it("should not revert or update compBorrowState index if cToken not in COMP markets", async () => {
      ...
      expect(block).toEqualNumber(100);
      const supplySpeed = await call(comptroller, "compSupplySpeeds", [
        mkt._address
      ]);
      expect(supplySpeed).toEqualNumber(0);
      const borrowSpeed = await call(comptroller, "compBorrowSpeeds", [
        mkt._address
      ]);
  ...
  describe("updateCompSupplyIndex()", () => {
    ...
    it("should not matter if the index is updated multiple times", async () => {
      ...
      await send(
        comptroller.comp,
        "transfer",
        [comptroller._address, compRemaining],
        { from: root }
      );
      ...
      const txT1 = await send(
        cLOW,
        "transfer",
        [a2, a3Balance0.minus(a2Balance0)],
        { from: a3 }
      );
      ...
      const txT2 = await send(
        cLOW,
        "transfer",
        [a3, a2Balance1.minus(a3Balance1)],
        { from: a2 }
      );
  ...
  describe("distributeBorrowerComp()", () => {
    it("should update borrow index checkpoint but not compAccrued for first time user", async () => {
      const mkt = cREP;
      await send(comptroller, "setCompBorrowState", [
        mkt._address,
        etherDouble(6),
        10
      ]);
      await send(comptroller, "setCompBorrowerIndex", [
        mkt._address,
        root,
        etherUnsigned(0)
      ]);

      await send(comptroller, "harnessDistributeBorrowerComp", [
        mkt._address,
        root,
        etherExp(1.1)
      ]);
      ...
      expect(
        await call(comptroller, "compBorrowerIndex", [mkt._address, root])
      ).toEqualNumber(6e36);
    });
    ...
    it("should transfer comp and update borrow index checkpoint correctly for repeat time user", async () => {
      ...
      */
      const tx = await send(comptroller, "harnessDistributeBorrowerComp", [
        mkt._address,
        a1,
        etherUnsigned(1.1e18)
      ]);
    ...
    it("should not revert or distribute when called with non-COMP market", async () => {
      ...
      expect(await compBalance(comptroller, a1)).toEqualNumber(0);
      expect(
        await call(comptroller, "compBorrowerIndex", [mkt._address, a1])
      ).toEqualNumber(compInitialIndex);
    });
  });
  ...
  describe("distributeSupplierComp()", () => {
    it("should transfer comp and update supply index correctly for first time user", async () => {
      ...
      await send(comptroller, "setCompSupplyState", [
        mkt._address,
        etherDouble(6),
        10
      ]);
      ...
      */
      ...
      const tx = await send(comptroller, "harnessDistributeAllSupplierComp", [
        mkt._address,
        a1
      ]);
      expect(await compAccrued(comptroller, a1)).toEqualNumber(0);
    ...
    it("should update comp accrued and supply index for repeat user", async () => {
      ...
      await send(comptroller, "setCompSupplierIndex", [
        mkt._address,
        a1,
        etherDouble(2)
      ]);
      ...
      */
      ...
      await send(comptroller, "harnessDistributeAllSupplierComp", [
        mkt._address,
        a1
      ]);
      expect(await compAccrued(comptroller, a1)).toEqualNumber(0);
    ...
    it("should not transfer when compAccrued below threshold", async () => {
      ...
      */
      ...
      await send(comptroller, "harnessDistributeSupplierComp", [
        mkt._address,
        a1
      ]);
      expect(await compAccrued(comptroller, a1)).toEqualNumber(0.00095e18);
  ...
  describe("transferComp", () => {
    it("should transfer comp accrued when amount is above threshold", async () => {
      ...
      const tx2 = await send(comptroller, "harnessTransferComp", [
        a1,
        a1AccruedPre,
        threshold
      ]);
  ...
  describe("claimComp", () => {
    it("should accrue comp and then transfer comp accrued", async () => {
      const compRemaining = compRate.multipliedBy(100),
        mintAmount = etherUnsigned(12e18),
        deltaBlocks = 10;
      await send(
        comptroller.comp,
        "transfer",
      ...
      expect(compBalancePre).toEqualNumber(0);
      expect(compBalancePost).toEqualNumber(
        compRate.multipliedBy(deltaBlocks).minus(1)
      ); // index is 8333...
    });
  ...
  describe("claimComp batch", () => {
    it("should revert when claiming comp from non-listed market", async () => {
      ...
      for (let from of claimAccts) {
        expect(
          await send(cLOW.underlying, "harnessSetBalance", [from, mintAmount], {
            from
          })
        ).toSucceed();
      ...
      await expect(
        send(comptroller, "claimComp", [
          claimAccts,
          [cLOW._address, cEVIL._address],
          true,
          true
        ])
      ).rejects.toRevert("revert market must be listed");
    ...
    it("should claim the expected amount when holders and ctokens arg is duplicated", async () => {
      ...
      const tx = await send(comptroller, "claimComp", [
        [...claimAccts, ...claimAccts],
        [cLOW._address, cLOW._address],
        false,
        true
      ]);
      ...
      for (let acct of claimAccts) {
        expect(
          await call(comptroller, "compSupplierIndex", [cLOW._address, acct])
        ).toEqualNumber(etherDouble(1.125));
        expect(await compBalance(comptroller, acct)).toEqualNumber(
          etherExp(1.25)
        );
      }
    });
    ...
    it("claims comp for multiple borrowers only, primes uninitiated", async () => {
      const compRemaining = compRate.multipliedBy(100),
        deltaBlocks = 10,
        mintAmount = etherExp(10),
        borrowAmt = etherExp(1),
        borrowIdx = etherExp(1);
      await send(
        comptroller.comp,
        "transfer",
        [comptroller._address, compRemaining],
      ...
      for (let acct of claimAccts) {
        ...
        await send(cLOW, "harnessSetAccountBorrows", [
          acct,
          borrowAmt,
          borrowIdx
        ]);
      ...
      for (let acct of claimAccts) {
        expect(
          await call(comptroller, "compBorrowerIndex", [cLOW._address, acct])
        ).toEqualNumber(etherDouble(2.25));
        expect(
          await call(comptroller, "compSupplierIndex", [cLOW._address, acct])
        ).toEqualNumber(0);
      }
  ...
  describe("harnessRefreshCompSpeeds", () => {
    ...
    it("should get correct speeds for 2 assets", async () => {
      ...
      await send(comptroller, "harnessAddCompMarkets", [
        [cLOW._address, cZRX._address]
      ]);
      await send(comptroller, "harnessRefreshCompSpeeds");
      const supplySpeed1 = await call(comptroller, "compSupplySpeeds", [
        cLOW._address
      ]);
      const borrowSpeed1 = await call(comptroller, "compBorrowSpeeds", [
        cLOW._address
      ]);
      const supplySpeed2 = await call(comptroller, "compSupplySpeeds", [
        cREP._address
      ]);
      const borrowSpeed2 = await call(comptroller, "compBorrowSpeeds", [
        cREP._address
      ]);
      const supplySpeed3 = await call(comptroller, "compSupplySpeeds", [
        cZRX._address
      ]);
      const borrowSpeed3 = await call(comptroller, "compBorrowSpeeds", [
        cZRX._address
      ]);
  ...
  describe("harnessSetCompSpeeds", () => {
    it("should correctly set differing COMP supply and borrow speeds", async () => {
      ...
      const tx = await send(comptroller, "_setCompSpeeds", [
        [cLOW._address],
        [desiredCompSupplySpeed],
        [desiredCompBorrowSpeed]
      ]);
      expect(tx).toHaveLog(["CompBorrowSpeedUpdated", 0], {
        cToken: cLOW._address,
      ...
      const currentCompSupplySpeed = await call(
        comptroller,
        "compSupplySpeeds",
        [cLOW._address]
      );
      const currentCompBorrowSpeed = await call(
        comptroller,
        "compBorrowSpeeds",
        [cLOW._address]
      );
    ...
    it("should correctly get differing COMP supply and borrow speeds for 4 assets", async () => {
      ...
      await send(comptroller, "harnessAddCompMarkets", [
        [cREP._address, cZRX._address, cBAT._address, cDAI._address]
      ]);
      await send(comptroller, "_setCompSpeeds", [
        [cREP._address, cZRX._address, cBAT._address, cDAI._address],
        [supplySpeed1, supplySpeed2, supplySpeed3, supplySpeed4],
        [borrowSpeed1, borrowSpeed2, borrowSpeed3, borrowSpeed4]
      ]);

      const currentSupplySpeed1 = await call(comptroller, "compSupplySpeeds", [
        cREP._address
      ]);
      const currentBorrowSpeed1 = await call(comptroller, "compBorrowSpeeds", [
        cREP._address
      ]);
      const currentSupplySpeed2 = await call(comptroller, "compSupplySpeeds", [
        cZRX._address
      ]);
      const currentBorrowSpeed2 = await call(comptroller, "compBorrowSpeeds", [
        cZRX._address
      ]);
      const currentSupplySpeed3 = await call(comptroller, "compSupplySpeeds", [
        cBAT._address
      ]);
      const currentBorrowSpeed3 = await call(comptroller, "compBorrowSpeeds", [
        cBAT._address
      ]);
      const currentSupplySpeed4 = await call(comptroller, "compSupplySpeeds", [
        cDAI._address
      ]);
      const currentBorrowSpeed4 = await call(comptroller, "compBorrowSpeeds", [
        cDAI._address
      ]);
    ...
    const checkAccrualsBorrowAndSupply = async (
      ...
      const mintAmount = etherUnsigned(1000e18),
        borrowAmount = etherUnsigned(1e18),
        borrowCollateralAmount = etherUnsigned(1000e18),
        compRemaining = compRate.multipliedBy(100),
        deltaBlocks = 10;
      ...
      // Accrue COMP
      await send(comptroller, "claimComp", [
        [a1, a2],
        [cLOW._address],
        true,
        true
      ...
      expect(a1TotalCompPost).toEqualNumber(
        Number(compBorrowSpeed) > 0
          ? compBorrowSpeed.multipliedBy(deltaBlocks).minus(1)
          : 0
      );

      // check accrual for supply
      expect(a2TotalCompPost).toEqualNumber(
        Number(compSupplySpeed) > 0
          ? compSupplySpeed.multipliedBy(deltaBlocks)
          : 0
      );
    };
  ...
  describe("harnessAddCompMarkets", () => {
    it("should correctly add a comp market if called by admin", async () => {
      const cBAT = await makeCToken({ comptroller, supportMarket: true });
      const tx1 = await send(comptroller, "harnessAddCompMarkets", [
        [cLOW._address, cREP._address, cZRX._address]
      ]);
      const tx2 = await send(comptroller, "harnessAddCompMarkets", [
        [cBAT._address]
      ]);
      const markets = await call(comptroller, "getCompMarkets");
  ...
  describe("updateContributorRewards", () => {
    ...
    it("should accrue comp to contributors", async () => {
      const tx1 = await send(comptroller, "_setContributorCompSpeed", [
        a1,
        2000
      ]);
      ...
      const tx2 = await send(comptroller, "updateContributorRewards", [a1], {
        from: a1
      });
  ...
  describe("_setContributorCompSpeed", () => {
    ...
    it("should start comp stream if called by admin", async () => {
      const tx = await send(comptroller, "_setContributorCompSpeed", [
        a1,
        1000
      ]);
    ...
    it("should reset comp stream if set to 0", async () => {
      ...
      const tx3 = await send(comptroller, "updateContributorRewards", [a1], {
        from: a1
      });
      const a1Accrued = await compAccrued(comptroller, a1);
