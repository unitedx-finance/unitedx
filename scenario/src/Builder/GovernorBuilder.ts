...
export async function buildGovernor(
  ...
  const fetchers = [
    new Fetcher<{ name: StringV; timelock: AddressV; comp: AddressV; guardian: AddressV }, GovernorData>(
      `
      #### GovernorAlpha
    ...
    `,
      'Alpha',
      [new Arg('name', getStringV), new Arg('timelock', getAddressV), new Arg('comp', getAddressV), new Arg('guardian', getAddressV)],
      async (world, { name, timelock, comp, guardian }) => {
        return {
          invokation: await GovernorAlphaContract.deploy<Governor>(world, from, [timelock.val, comp.val, guardian.val]),
          name: name.val,
    ...
    `,
      ...
      async (world, { name, timelock, comp, guardian }) => {
        return {
          invokation: await GovernorAlphaHarnessContract.deploy<Governor>(world, from, [timelock.val, comp.val, guardian.val]),
          name: name.val,
  ...
  let govData = await getFetcherValue<any, GovernorData>('DeployGovernor', fetchers, world, params);
  ...
  world = await storeAndSaveContract(world, governor, govData.name, invokation, [{ index: ['Governor', govData.name], data: govData }]);
