...
import { getAddressV, getExpNumberV, getNumberV, getPercentV, getStringV } from '../CoreValue';
...
export async function buildInterestRateModel(
  ...
  from: string,
  event: Event
): Promise<{ world: World; interestRateModel: InterestRateModel; interestRateModelData: InterestRateModelData }> {
  const fetchers = [
    ...
    new Fetcher<{ name: StringV; baseRate: NumberV; multiplier: NumberV }, InterestRateModelData>(
      ...
      `,
      'WhitePaper',
      [new Arg('name', getStringV), new Arg('baseRate', getExpNumberV), new Arg('multiplier', getExpNumberV)],
    ...
    new Fetcher<{ name: StringV; baseRate: NumberV; multiplier: NumberV; jump: NumberV; kink: NumberV }, InterestRateModelData>(
      ...
      async (world, { name, baseRate, multiplier, jump, kink }) => ({
        ...
        description: `JumpRateModel baseRate=${baseRate
          .encode()
          .toString()} multiplier=${multiplier.encode().toString()} jump=${jump.encode().toString()} kink=${kink.encode().toString()}`,
        base: baseRate.encode().toString(),
        slope: multiplier.encode().toString(),
    ...
    new Fetcher<
      { name: StringV; baseRate: NumberV; multiplier: NumberV; jump: NumberV; kink: NumberV; owner: AddressV },
    ...
    >(
      ...
      async (world, { name, baseRate, multiplier, jump, kink, owner }) => ({
        invokation: await JumpRateModelV2.deploy<InterestRateModel>(world, from, [
          baseRate.encode(),
          multiplier.encode(),
          jump.encode(),
          kink.encode(),
    ...
    >(
      ...
      async (world, { name, baseRate, multiplier, jump, kink, owner }) => ({
        invokation: await LegacyJumpRateModelV2.deploy<InterestRateModel>(world, from, [
          baseRate.encode(),
          multiplier.encode(),
          jump.encode(),
          kink.encode(),
    ...
    new Fetcher<{ name: StringV; jump: NumberV; kink: NumberV; pot: AddressV; jug: AddressV; owner: AddressV }, InterestRateModelData>(
      ...
      async (world, { name, jump, kink, pot, jug, owner }) => ({
        invokation: await DAIInterestRateModel.deploy<InterestRateModel>(world, from, [
          jump.encode(),
          kink.encode(),
          pot.val,
          jug.val,
        ...
        description: `DAIInterestRateModel jump=${jump.encode().toString()} kink=${kink.encode().toString()} pot=${pot.val} jug=${
          jug.val
        } owner=${owner.val}`,
        jump: jump.encode().toString(),
        kink: kink.encode().toString(),
  ...
  world = await storeAndSaveContract(world, interestRateModel, interestRateModelData.name, invokation, [
    {
