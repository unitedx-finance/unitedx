  ...
  return async world => {
    ...
    async function build<T extends Contract>(
      ...
      const fetchers = constructors.map((abi: any) => {
        let nameArg = implicit ? [] : [new Arg('name', getStringV, { default: new StringV(contractName) })];
    ...
    async function deploy<T extends Contract>(world: World, from: string, params: Event) {
      ...
      world = addAction(world, `Deployed ${contractName} ${data.contract} to address ${contract._address}`, data.invokation);
    ...
    function commands<T extends Contract>() {
      async function buildOutput(world: World, from: string, fn: string, inputs: object, output: AbiItem): Promise<World> {
        ...
        world = addAction(world, `Invokation of ${fn} with inputs ${inputs}`, invokation);
      ...
      let abiCommands = abis
        ...
        .map((abi: any) => {
          ...
          let inputNames = abi.inputs.map(input => getEventName(input.name));
          let args = [new Arg('contract', getContractObjectFn(contractName, implicit), implicit ? { implicit: true } : {})].concat(
