...
import { getAddressV, getEventV, getExpNumberV, getNumberV, getStringV, getBoolV } from '../CoreValue';
...
async function accrueInterest(world: World, from: string, cToken: CToken): Promise<World> {
  ...
  world = addAction(world, `CToken ${cToken.name}: Interest accrued`, invokation);
...
async function mint(world: World, from: string, cToken: CToken, amount: NumberV | NothingV): Promise<World> {
  ...
  world = addAction(world, `CToken ${cToken.name}: ${describeUser(world, from)} mints ${showAmount}`, invokation);
...
async function redeem(world: World, from: string, cToken: CToken, tokens: NumberV): Promise<World> {
  ...
  world = addAction(world, `CToken ${cToken.name}: ${describeUser(world, from)} redeems ${tokens.show()} tokens`, invokation);
...
async function redeemUnderlying(world: World, from: string, cToken: CToken, amount: NumberV): Promise<World> {
  ...
  world = addAction(world, `CToken ${cToken.name}: ${describeUser(world, from)} redeems ${amount.show()} underlying`, invokation);
...
async function borrow(world: World, from: string, cToken: CToken, amount: NumberV): Promise<World> {
  ...
  world = addAction(world, `CToken ${cToken.name}: ${describeUser(world, from)} borrows ${amount.show()}`, invokation);
...
async function repayBorrow(world: World, from: string, cToken: CToken, amount: NumberV | NothingV): Promise<World> {
  ...
  world = addAction(world, `CToken ${cToken.name}: ${describeUser(world, from)} repays ${showAmount} of borrow`, invokation);
...
async function liquidateBorrow(
  ...
  if (repayAmount instanceof NumberV) {
    ...
    invokation = await invoke(
      world,
      cToken.methods.liquidateBorrow(borrower, repayAmount.encode(), collateral._address),
      from,
      CTokenErrorReporter
    );
  } else {
  ...
  world = addAction(
    world,
    `CToken ${cToken.name}: ${describeUser(world, from)} liquidates ${showAmount} from of ${describeUser(world, borrower)}, seizing ${
      collateral.name
    }.`,
    invokation
  );
...
async function seize(
  ...
  world = addAction(
    world,
    `CToken ${cToken.name}: ${describeUser(world, from)} initiates seizing ${seizeTokens.show()} to ${describeUser(
      world,
      liquidator
...
async function evilSeize(
  ...
  let invokation = await invoke(
    world,
    cToken.methods.evilSeize(treasure._address, liquidator, borrower, seizeTokens.encode()),
    from,
    CTokenErrorReporter
  );
  ...
  world = addAction(
    world,
    `CToken ${cToken.name}: ${describeUser(world, from)} initiates illegal seizing ${seizeTokens.show()} to ${describeUser(
      world,
      liquidator
    )} from ${describeUser(world, borrower)}.`,
    invokation
...
async function setPendingAdmin(world: World, from: string, cToken: CToken, newPendingAdmin: string): Promise<World> {
  ...
  world = addAction(world, `CToken ${cToken.name}: ${describeUser(world, from)} sets pending admin to ${newPendingAdmin}`, invokation);
...
async function acceptAdmin(world: World, from: string, cToken: CToken): Promise<World> {
  ...
  world = addAction(world, `CToken ${cToken.name}: ${describeUser(world, from)} accepts admin`, invokation);
...
async function addReserves(world: World, from: string, cToken: CToken, amount: NumberV): Promise<World> {
  ...
  world = addAction(world, `CToken ${cToken.name}: ${describeUser(world, from)} adds to reserves by ${amount.show()}`, invokation);
...
async function reduceReserves(world: World, from: string, cToken: CToken, amount: NumberV): Promise<World> {
  ...
  world = addAction(world, `CToken ${cToken.name}: ${describeUser(world, from)} reduces reserves by ${amount.show()}`, invokation);
...
async function setInterestRateModel(world: World, from: string, cToken: CToken, interestRateModel: string): Promise<World> {
  ...
  world = addAction(world, `Set interest rate for ${cToken.name} to ${interestRateModel} as ${describeUser(world, from)}`, invokation);
...
async function setComptroller(world: World, from: string, cToken: CToken, comptroller: string): Promise<World> {
  ...
  world = addAction(world, `Set comptroller for ${cToken.name} to ${comptroller} as ${describeUser(world, from)}`, invokation);
...
async function becomeImplementation(world: World, from: string, cToken: CToken, becomeImplementationData: string): Promise<World> {
  ...
  world = addAction(
    world,
    `CToken ${cToken.name}: ${describeUser(world, from)} initiates _becomeImplementation with data:${becomeImplementationData}.`,
    invokation
  );
...
async function resignImplementation(world: World, from: string, cToken: CToken): Promise<World> {
  const cErc20Delegate = getContract('CErc20Delegate');
  const cErc20DelegateContract = await cErc20Delegate.at<CErc20Delegate>(world, cToken._address);

  let invokation = await invoke(world, cErc20DelegateContract.methods._resignImplementation(), from, CTokenErrorReporter);

  world = addAction(world, `CToken ${cToken.name}: ${describeUser(world, from)} initiates _resignImplementation.`, invokation);
...
async function setImplementation(
  ...
  let invokation = await invoke(
    world,
    cToken.methods._setImplementation(implementation, allowResign, becomeImplementationData),
    from,
    CTokenErrorReporter
  );
...
async function donate(world: World, from: string, cToken: CToken): Promise<World> {
  ...
  world = addAction(world, `Donate for ${cToken.name} as ${describeUser(world, from)} with value ${showTrxValue(world)}`, invokation);
...
async function setCTokenMock(world: World, from: string, cToken: CTokenScenario, mock: string, value: NumberV): Promise<World> {
  ...
  world = addAction(world, `Mocked ${mock}=${value.show()} for ${cToken.name}`, invokation);
...
export function cTokenCommands() {
  return [
    ...
    new Command<{ cToken: CToken; behalf: AddressV; amount: NumberV | NothingV }>(
      ...
      `,
      'RepayBorrowBehalf',
      [new Arg('cToken', getCTokenV), new Arg('behalf', getAddressV), new Arg('amount', getNumberV, { nullable: true })],
    ...
    new Command<{ borrower: AddressV; cToken: CToken; collateral: CToken; repayAmount: NumberV | NothingV }>(
      ...
      (world, from, { borrower, cToken, collateral, repayAmount }) =>
        liquidateBorrow(world, from, cToken, borrower.val, collateral, repayAmount),
      { namePos: 1 }
    ),
    ...
    new Command<{ cToken: CToken; treasure: CToken; liquidator: AddressV; borrower: AddressV; seizeTokens: NumberV }>(
      ...
      (world, from, { cToken, treasure, liquidator, borrower, seizeTokens }) =>
        evilSeize(world, from, cToken, treasure, liquidator.val, borrower.val, seizeTokens),
      { namePos: 1 }
    ),
    new Command<{ cToken: CToken; amount: NumberV }>(
      `
  ...
  `,
    ...
    new Command<{ cToken: CToken; interestRateModel: AddressV }>(
      ...
      `,
      'SetInterestRateModel',
      [new Arg('cToken', getCTokenV), new Arg('interestRateModel', getAddressV)],
    ...
    new Command<{
      ...
      `,
      'BecomeImplementation',
      [new Arg('cToken', getCTokenV), new Arg('becomeImplementationData', getStringV)],
      (world, from, { cToken, becomeImplementationData }) => becomeImplementation(world, from, cToken, becomeImplementationData.val),
      { namePos: 1 }
    ),
    new Command<{ cToken: CToken }>(
      `
    ...
    new Command<{
      ...
      (world, from, { cToken, implementation, allowResign, becomeImplementationData }) =>
        setImplementation(world, from, cToken, implementation.val, allowResign.val, becomeImplementationData.val),
      { namePos: 1 }
    ),
    new Command<{ cToken: CToken }>(
      `
    ...
    new Command<{ cToken: CToken; variable: StringV; value: NumberV }>(
      ...
      `,
      'Mock',
      [new Arg('cToken', getCTokenV), new Arg('variable', getStringV), new Arg('value', getNumberV)],
