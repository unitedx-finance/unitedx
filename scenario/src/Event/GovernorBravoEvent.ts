...
import { getAddressV, getArrayV, getEventV, getNumberV, getStringV, getCoreValue } from '../CoreValue';
...
async function genGovernor(world: World, from: string, params: Event): Promise<World> {
  ...
  return addAction(world, `Deployed GovernorBravo ${govData.contract} to address ${governor._address}`, govData.invokation);
}
...
async function mergeABI(world: World, from: string, governorDelegator: GovernorBravo, governorDelegate: GovernorBravo): Promise<World> {
  if (!world.dryRun) {
    // Skip this specifically on dry runs since it's likely to crash due to a number of reasons
    world = await mergeContractABI(world, 'GovernorBravo', governorDelegator, governorDelegator.name, governorDelegate.name);
  }
...
async function setVotingDelay(world: World, from: string, governor: GovernorBravo, newVotingDelay: NumberV): Promise<World> {
  let invokation = await invoke(world, governor.methods._setVotingDelay(newVotingDelay.encode()), from);
...
async function setVotingPeriod(world: World, from: string, governor: GovernorBravo, newVotingPeriod: NumberV): Promise<World> {
  let invokation = await invoke(world, governor.methods._setVotingPeriod(newVotingPeriod.encode()), from);
...
async function setWhitelistAccountExpiration(
  ...
  let invokation = await invoke(world, governor.methods._setWhitelistAccountExpiration(account, expiration.encode()), from);

  world = addAction(world, `Set account whitelist expiration for ${account} to ${expiration.encode()}`, invokation);

  return world;
}

async function setWhitelistGuardian(world: World, from: string, governor: GovernorBravo, account: string): Promise<World> {
  let invokation = await invoke(world, governor.methods._setWhitelistGuardian(account), from);

  world = addAction(world, `Set whitelist guardian to ${account}`, invokation);

  return world;
}

async function setImplementation(world: World, from: string, governor: GovernorBravo, newImplementation: GovernorBravo): Promise<World> {
  let invokation = await invoke(world, governor.methods._setImplementation(newImplementation._address), from);
...
async function setBlockNumber(world: World, from: string, governor: GovernorBravo, blockNumber: NumberV): Promise<World> {
  return addAction(
    world,
...
async function setBlockTimestamp(world: World, from: string, governor: GovernorBravo, blockTimestamp: NumberV): Promise<World> {
  return addAction(
    world,
...
export function governorBravoCommands() {
  return [
    ...
    `,
      ...
      (world, from, { governor, newProposalThreshold }) => setProposalThreshold(world, from, governor, newProposalThreshold),
      { namePos: 1 }
    ),
    ...
    `,
      'SetWhitelistAccountExpiration',
      [new Arg('governor', getGovernorV), new Arg('address', getAddressV), new Arg('expiration', getNumberV)],
      (world, from, { governor, address, expiration }) => setWhitelistAccountExpiration(world, from, governor, address.val, expiration),
      { namePos: 1 }
    ),
    new Command<{ governor: GovernorBravo; address: AddressV; expiration: NumberV }>(
      `
    ...
    `,
      'SetWhitelistGuardian',
      [new Arg('governor', getGovernorV), new Arg('address', getAddressV)],
      (world, from, { governor, address, expiration }) => setWhitelistGuardian(world, from, governor, address.val),
      { namePos: 1 }
    ),
    ...
    new Command<{
      ...
      (world, from, { governorDelegator, governorDelegate }) => mergeABI(world, from, governorDelegator, governorDelegate),
      { namePos: 1 }
    )
  ];
}
