...
async function genPriceOracleProxy(world: World, from: string, params: Event): Promise<World> {
  ...
  world = addAction(world, `Deployed PriceOracleProxy to address ${priceOracleProxy._address}`, invokation);
...
export function priceOracleProxyCommands() {
  return [
    ...
    new Command<{ priceOracleProxy: PriceOracleProxy; amount: NumberV }>(
      ...
      `,
      'SetSaiPrice',
      [new Arg('priceOracleProxy', getPriceOracleProxy, { implicit: true }), new Arg('amount', getExpNumberV)],
