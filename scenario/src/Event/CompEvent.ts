...
async function genComp(world: World, from: string, params: Event): Promise<World> {
  ...
  world = addAction(world, `Deployed Comp (${comp.name}) to address ${comp._address}`, tokenData.invokation);
...
async function approve(world: World, from: string, comp: Comp, address: string, amount: NumberV): Promise<World> {
  ...
  world = addAction(world, `Approved Comp token for ${from} of ${amount.show()}`, invokation);
...
async function transfer(world: World, from: string, comp: Comp, address: string, amount: NumberV): Promise<World> {
  ...
  world = addAction(world, `Transferred ${amount.show()} Comp tokens from ${from} to ${address}`, invokation);
...
async function transferFrom(world: World, from: string, comp: Comp, owner: string, spender: string, amount: NumberV): Promise<World> {
  ...
  world = addAction(world, `"Transferred from" ${amount.show()} Comp tokens from ${owner} to ${spender}`, invokation);
...
async function transferScenario(world: World, from: string, comp: CompScenario, addresses: string[], amount: NumberV): Promise<World> {
  ...
  world = addAction(world, `Transferred ${amount.show()} Comp tokens from ${from} to ${addresses}`, invokation);
...
async function transferFromScenario(world: World, from: string, comp: CompScenario, addresses: string[], amount: NumberV): Promise<World> {
  ...
  world = addAction(world, `Transferred ${amount.show()} Comp tokens from ${addresses} to ${from}`, invokation);
...
async function setBlockNumber(world: World, from: string, comp: Comp, blockNumber: NumberV): Promise<World> {
  return addAction(
    world,
...
export function compCommands() {
  return [
    ...
    new Command<{ comp: Comp; spender: AddressV; amount: NumberV }>(
      ...
      `,
      'Approve',
      [new Arg('comp', getComp, { implicit: true }), new Arg('spender', getAddressV), new Arg('amount', getNumberV)],
    ...
    new Command<{ comp: Comp; recipient: AddressV; amount: NumberV }>(
      ...
      `,
      'Transfer',
      [new Arg('comp', getComp, { implicit: true }), new Arg('recipient', getAddressV), new Arg('amount', getNumberV)],
    ...
    new Command<{ comp: CompScenario; recipients: AddressV[]; amount: NumberV }>(
      ...
      `,
      'TransferScenario',
      [new Arg('comp', getComp, { implicit: true }), new Arg('recipients', getAddressV, { mapped: true }), new Arg('amount', getNumberV)],
    ...
    new Command<{ comp: CompScenario; froms: AddressV[]; amount: NumberV }>(
      ...
      `,
      'TransferFromScenario',
      [new Arg('comp', getComp, { implicit: true }), new Arg('froms', getAddressV, { mapped: true }), new Arg('amount', getNumberV)],
    ...
    new Command<{ comp: Comp; account: AddressV }>(
      ...
      `,
      'Delegate',
      [new Arg('comp', getComp, { implicit: true }), new Arg('account', getAddressV)],
