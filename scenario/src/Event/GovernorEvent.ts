...
async function genGovernor(world: World, from: string, params: Event): Promise<World> {
  ...
  return addAction(world, `Deployed Governor ${govData.contract} to address ${governor._address}`, govData.invokation);
}
...
async function setBlockNumber(world: World, from: string, governor: Governor, blockNumber: NumberV): Promise<World> {
  return addAction(
    world,
...
async function setBlockTimestamp(world: World, from: string, governor: Governor, blockTimestamp: NumberV): Promise<World> {
  return addAction(
    world,
