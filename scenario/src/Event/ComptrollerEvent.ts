...
import { getAddressV, getBoolV, getEventV, getExpNumberV, getNumberV, getPercentV, getStringV, getCoreValue } from '../CoreValue';
...
async function genComptroller(world: World, from: string, params: Event): Promise<World> {
  let { world: nextWorld, comptrollerImpl: comptroller, comptrollerImplData: comptrollerData } = await buildComptrollerImpl(
    world,
    from,
    params
  );
...
async function setPaused(world: World, from: string, comptroller: Comptroller, actionName: string, isPaused: boolean): Promise<World> {
  ...
  world = addAction(world, `Comptroller: set paused for ${actionName} to ${isPaused}`, invokation);
...
async function setCollateralFactor(
  ...
  let invokation = await invoke(
    world,
    comptroller.methods._setCollateralFactor(cToken._address, collateralFactor.encode()),
    from,
    ComptrollerErrorReporter
  );

  world = addAction(world, `Set collateral factor for ${cToken.name} to ${collateralFactor.show()}`, invokation);
...
async function addCompMarkets(world: World, from: string, comptroller: Comptroller, cTokens: CToken[]): Promise<World> {
  ...
  world = addAction(world, `Added COMP markets ${cTokens.map(c => c.name)}`, invokation);
...
async function claimCompInMarkets(world: World, from: string, comptroller: Comptroller, holder: string, cTokens: CToken[]): Promise<World> {
  ...
  world = addAction(world, `Comp claimed by ${holder} in markets ${cTokens.map(c => c.name)}`, invokation);
...
async function updateContributorRewards(world: World, from: string, comptroller: Comptroller, contributor: string): Promise<World> {
  ...
  world = addAction(world, `Contributor rewards updated for ${contributor}`, invokation);
...
async function grantComp(world: World, from: string, comptroller: Comptroller, recipient: string, amount: NumberV): Promise<World> {
  ...
  world = addAction(world, `${amount.show()} comp granted to ${recipient}`, invokation);
...
async function setCompSpeed(world: World, from: string, comptroller: Comptroller, cToken: CToken, speed: NumberV): Promise<World> {
  ...
  world = addAction(world, `Comp speed for market ${cToken._address} set to ${speed.show()}`, invokation);

  return world;
}

async function setCompSpeeds(
  world: World,
  from: string,
  comptroller: Comptroller,
  cTokens: CToken[],
  supplySpeeds: NumberV[],
  borrowSpeeds: NumberV[]
): Promise<World> {
  let invokation = await invoke(
    world,
    comptroller.methods._setCompSpeeds(
      cTokens.map(c => c._address),
      supplySpeeds.map(speed => speed.encode()),
      borrowSpeeds.map(speed => speed.encode())
    ),
    from,
    ComptrollerErrorReporter
  );
  ...
  world = addAction(
    world,
    `Comp speed for markets [${cTokens.map(c => c._address)}] set to supplySpeeds=[${supplySpeeds.map(speed =>
      speed.show()
    )}, borrowSpeeds=[${borrowSpeeds.map(speed => speed.show())}]`,
    invokation
  );
...
async function setContributorCompSpeed(
  ...
  let invokation = await invoke(
    world,
    comptroller.methods._setContributorCompSpeed(contributor, speed.encode()),
    from,
    ComptrollerErrorReporter
  );

  world = addAction(world, `Comp speed for contributor ${contributor} set to ${speed.show()}`, invokation);
...
async function setPendingAdmin(world: World, from: string, comptroller: Comptroller, newPendingAdmin: string): Promise<World> {
  ...
  world = addAction(world, `Comptroller: ${describeUser(world, from)} sets pending admin to ${newPendingAdmin}`, invokation);
...
async function setPauseGuardian(world: World, from: string, comptroller: Comptroller, newPauseGuardian: string): Promise<World> {
  ...
  world = addAction(world, `Comptroller: ${describeUser(world, from)} sets pause guardian to ${newPauseGuardian}`, invokation);
...
async function setGuardianPaused(world: World, from: string, comptroller: Comptroller, action: string, state: boolean): Promise<World> {
  ...
  world = addAction(world, `Comptroller: ${describeUser(world, from)} sets ${action} paused`, invokation);
...
async function setMarketBorrowCaps(
  ...
  world = addAction(world, `Borrow caps on ${cTokens} set to ${borrowCaps}`, invokation);
...
async function setBorrowCapGuardian(world: World, from: string, comptroller: Comptroller, newBorrowCapGuardian: string): Promise<World> {
  ...
  world = addAction(world, `Comptroller: ${describeUser(world, from)} sets borrow cap guardian to ${newBorrowCapGuardian}`, invokation);
...
export function comptrollerCommands() {
  return [
    ...
    new Command<{ comptroller: Comptroller; cToken: CToken }>(
      ...
      `,
      'SupportMarket',
      [new Arg('comptroller', getComptroller, { implicit: true }), new Arg('cToken', getCTokenV)],
    ...
    new Command<{ comptroller: Comptroller; cTokens: CToken[] }>(
      ...
      `,
      'EnterMarkets',
      [new Arg('comptroller', getComptroller, { implicit: true }), new Arg('cTokens', getCTokenV, { mapped: true })],
    ...
    new Command<{ comptroller: Comptroller; newPauseGuardian: AddressV }>(
      ...
      `,
      'SetPauseGuardian',
      [new Arg('comptroller', getComptroller, { implicit: true }), new Arg('newPauseGuardian', getAddressV)],
    ...
    new Command<{ comptroller: Comptroller; cToken: CToken; action: StringV; isPaused: BoolV }>(
      ...
      (world, from, { comptroller, cToken, action, isPaused }) =>
        setGuardianMarketPaused(world, from, comptroller, cToken, action.val, isPaused.val)
    ),
    ...
    new Command<{ comptroller: Comptroller; blocks: NumberV; _keyword: StringV }>(
      ...
      `,
      'FastForward',
      [new Arg('comptroller', getComptroller, { implicit: true }), new Arg('blocks', getNumberV), new Arg('_keyword', getStringV)],
    ...
    new Command<{ comptroller: Comptroller; contributor: AddressV }>(
      ...
      * E.g. "Comptroller UpdateContributorRewards Geoff
      `,
      'UpdateContributorRewards',
      [new Arg('comptroller', getComptroller, { implicit: true }), new Arg('contributor', getAddressV)],
    ...
    new Command<{ comptroller: Comptroller; recipient: AddressV; amount: NumberV }>(
      ...
      * E.g. "Comptroller GrantComp Geoff 1e18
      `,
      'GrantComp',
      [new Arg('comptroller', getComptroller, { implicit: true }), new Arg('recipient', getAddressV), new Arg('amount', getNumberV)],
    ...
    new Command<{ comptroller: Comptroller; cToken: CToken; speed: NumberV }>(
      ...
      * E.g. "Comptroller SetCompSpeed cToken 1000
      `,
      'SetCompSpeed',
      [new Arg('comptroller', getComptroller, { implicit: true }), new Arg('cToken', getCTokenV), new Arg('speed', getNumberV)],
    ...
    new Command<{ comptroller: Comptroller; cTokens: CToken[]; supplySpeeds: NumberV[]; borrowSpeeds: NumberV[] }>(
      ...
      * E.g. "Comptroller SetCompSpeeds (cZRX cBAT) (1000 0) (1000 2000)
      ...
      (world, from, { comptroller, cTokens, supplySpeeds, borrowSpeeds }) =>
        setCompSpeeds(world, from, comptroller, cTokens, supplySpeeds, borrowSpeeds)
    ),
    new Command<{ comptroller: Comptroller; contributor: AddressV; speed: NumberV }>(
      ...
      * E.g. "Comptroller SetContributorCompSpeed contributor 1000
      `,
      'SetContributorCompSpeed',
      [new Arg('comptroller', getComptroller, { implicit: true }), new Arg('contributor', getAddressV), new Arg('speed', getNumberV)],
