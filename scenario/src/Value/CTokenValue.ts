...
export async function getCTokenV(world: World, event: Event): Promise<CToken> {
  const address = await mapValue<AddressV>(world, event, str => new AddressV(getCTokenAddress(world, str)), getCoreValue, AddressV);
...
export function cTokenFetchers() {
  return [
    ...
    new Fetcher<{ cToken: CToken; address: AddressV }, NumberV>(
      ...
      `,
      'UnderlyingBalance',
      [new Arg('cToken', getCTokenV), new Arg<AddressV>('address', getAddressV)],
