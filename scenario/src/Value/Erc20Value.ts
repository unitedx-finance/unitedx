...
export async function getErc20V(world: World, event: Event): Promise<Erc20> {
  const address = await mapValue<AddressV>(world, event, str => new AddressV(getErc20Address(world, str)), getCoreValue, AddressV);
...
export function erc20Fetchers() {
  return [
    ...
    new Fetcher<{ erc20: Erc20; owner: AddressV; spender: AddressV }, NumberV>(
      ...
      `,
      'Allowance',
      [new Arg('erc20', getErc20V), new Arg('owner', getAddressV), new Arg('spender', getAddressV)],
