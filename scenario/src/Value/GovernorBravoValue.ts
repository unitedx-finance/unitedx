...
export async function getGovernorV(world: World, event: Event): Promise<GovernorBravo> {
  const address = await mapValue<AddressV>(world, event, str => new AddressV(getGovernorAddress(world, str)), getCoreValue, AddressV);
...
async function getIsWhitelisted(world: World, governor: GovernorBravo, account: string): Promise<BoolV> {
