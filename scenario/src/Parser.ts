      ...
      switch (expectation.type) {
        ...
        case 'class':
          const escapedParts = expectation.parts.map(part => {
            return Array.isArray(part) ? classEscape(part[0] as string) + '-' + classEscape(part[1] as string) : classEscape(part);
          });
  ...
  function getArgValues(eventArgs, macroArgs) {
    ...
    eventArgs.forEach(eventArg => {
      if (eventArg.argName) {
        ...
        if (usedNamedArg) {
          throw new Error(
            `Cannot use positional arg after named arg in macro invokation ${JSON.stringify(eventArgs)} looking at ${eventArg.toString()}`
          );
        }
